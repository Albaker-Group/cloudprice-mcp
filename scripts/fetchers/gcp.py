...
def fetch_instance_prices(skus: list[InstanceSku]) -> list[InstanceSku]:
    ...
    for entry in skus:
        ...
        # Shared-core SKUs ship a fixed price each.
...
def _extract_family_rates(items: list[dict]) -> dict[str, tuple[float, float]]:
    """Walk the SKU list and find core/RAM rates per family.

    Skips: Custom (we don't price custom shapes), Preemptible/Spot (handled by
    the spot fetcher in v0.8.1+), Sole Tenant, Commitment, Premium, and any SKU
    that's not flagged as serving us-east1.
    """
    rates: dict[str, tuple[float | None, float | None]] = {
        family: (None, None) for family in _FAMILY_DESCRIPTION_PATTERNS
    }

    for sku in items:
        description = sku.get("description") or ""
        # Filter out variants we don't want.
        lowered = description.lower()
        if any(skip in lowered for skip in ("preemptible", "spot", "custom", "sole tenant", "commitment")):
            continue
        if _REGION_KEY not in (sku.get("serviceRegions") or []):
            continue

        for family, (core_pat, ram_pat) in _FAMILY_DESCRIPTION_PATTERNS.items():
